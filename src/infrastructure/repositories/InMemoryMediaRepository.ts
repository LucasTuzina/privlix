...
export class InMemoryMediaRepository implements IMediaRepository {
  ...
  constructor(private readonly fileSystemRepo: IFileSystemRepository) {}
  ...
  async scanFolder(folderPath: string): Promise<void> {
    ...
    // Setup file watcher
    this.fileSystemRepo.watchFolder(folderPath, (filePath, event) => {
  ...
  private async processMediaFile(filePath: string): Promise<void> {
    ...
    // Get file stats
    const fileStats = await this.fileSystemRepo.getFileStats(filePath);
